...
**Status:** Iterations 1-7 implemented, iteration 8 not started. Project paused (no Windows machine available). See README.md for the honest feature state.
...
## Post-Iteration-7 Fixes (manual testing, 2025-12-04/05)

First end-to-end test on a real Windows machine. Merged to `main` 2026-09-08 with the
matching test and contract updates.

- Hold-to-talk implemented with a low-level keyboard hook for key-up detection (US-001);
  previously recording stopped after a fixed 500 ms.
- `whisper-cli` invocation corrected to `-oj` / `-of` and parsing switched to the real
  whisper.cpp JSON (`result.language`, `transcription[]`). Contract updated in
  `docs/architecture/interface-contracts.md`.
- Tray icon now created explicitly (`ForceCreate`) and GDI icon handles released.
- Hotkey conflict at startup logs a warning instead of a blocking dialog.
- Wizard: default data root `%LOCALAPPDATA%\LocalWhisper`, folder auto-created, scroll and
  initialization-order fixes, no copy when the model already sits in the target folder,
  post-processing paths not validated during wizard.
- Settings: model SHA-1 verified against the known-model database; unknown models pass
  with a warning; a hash mismatch no longer blocks saving.
- Global exception handlers added (AppDomain, Dispatcher, TaskScheduler).
- Added `docs/MANUAL_SETUP_GUIDE.md`.

**Known rough edges after this round**
- Wizard does not ask for the `whisper-cli.exe` path; manual `config.toml` edit required.
- `ErrorDialog` "Settings" button is still a placeholder (PH-001).
- Post-processing requires the matching llama.cpp DLLs (CUDA or CPU build) next to `llama-cli.exe`.
- Nothing after 2025-12-05 has been run on Windows.
